...
class Editor:
    def __init__(self, land_tiles, switch):
        ...
        self.editor_music1 = pygame.mixer.Sound('../audio/Otherside.mp3')
        self.editor_music.set_volume(0.4)
        self.editor_music1.set_volume(0.4)
        ...
        if sam == 1:
            ...
            self.editor_music1.play(loops=-1)
            ...
            if cell in self.canvas_data:
                ...
                for name, side in NEIGHBOR_DIRECTIONS.items():
                    ...
                    if neighbor_cell in self.canvas_data:
                        # water top neighbor
                        if self.canvas_data[neighbor_cell].has_water and self.canvas_data[cell] \
                                .has_water and name == 'A':
    ...
    def canvas_add(self):
        if mouse_buttons()[0] and not self.menu.rect.collidepoint(mouse_pos()) \
                and not self.object_drag_active and Button.can_click:
    ...
    def loading(self):
        if Button.load_ie:
            ...
            # 使用 simpledialog 来创建对话框输入
            name = simpledialog.askstring("输入存档码", "请输入您的存档码: ")

            # 显示用户输入的名字
            if name:
                try:
                    self.switch(eval(name))
                except NameError:
                    display_text(self.display_surface, "存档码错误not found", 36, (255, 0, 0), (640, 50))

                self.editor_music.stop()
                self.editor_music1.stop()
            else:
                # 在这里添加一个循环来等待用户输入并显示文字
                display_text(self.display_surface, "您没有输入存档码", 36, (0, 0, 0), (640, 50))
                pygame.display.flip()  # 更新显示

            # 关闭主窗口
            root.destroy()

    # 在其他地方的循环中调用 loading 函数
    def run(self, dt):
        self.event_loop()
        ...
        if Button.active:
            pyperclip.copy(str(self.create_grid()))
            display_text(self.display_surface, "已将存档码复制到粘贴板", 36, (0, 0, 0), (640, 50), 1000)

        if Button.share:
            item1 = SaveGame("Item 1", str(self.create_grid()), "image1.jpg", 1)
            save_items_to_json([item1], "save.json")  # 使用列表保存单个实例

            # 从文件加载实例列表
            loaded_items = load_items_from_json("save.json")

            # 因为只有一个项目，所以加载的列表只有一个元素
            loaded_item = loaded_items[0]

            # 显示加载的实例信息
            loaded_item.display_info()

            display_text(self.display_surface, "114514", 36, (0, 0, 0), (640, 50), 500)
