...
lock = threading.Lock()
pygame.display.set_caption('Super Jump Man--LCL')
...
class Tooth(Generic):
    ...
    def shrink(self):
        self.image = load('../graphics/enemies/tooth/died/died E.png').convert_alpha()
        self.image = pygame.transform.scale(self.image, (204, 180))
    ...
    def animate(self, dt):
        ...
        if int(self.frame_index) == 2 and self.status == 'attack' and not self.has_shot:
            with lock:
                ...
                offset = (pearl_direction * 50) + vector(0, -10) \
...
class Player(Generic):
    ...
    def animate(self, dt):
        ...
        if self.invul_timer.active:
            ...
            self.image = surf
        if Player.xue <= 0:
            self.image = load('../graphics/player/ga/ga.png').convert_alpha()
